...
class TestCategories:
    def test_list_categories(self):
        ...
        assert "Eléctrico" in names
        assert "Mecánico" in names
        assert "Gasfitería" in names
        # Required fields
        for c in cats:
...
def _login(email, password):
    ...
    if r.status_code != 200:
        ...
        role = "technician" if "tech" in email.lower() or "tecnico" in email.lower() else "client"
...
class TestTechnicianProfile:
    ...
    def test_get_technician_profile(self):
        ...
        r = requests.get(f"{BASE_URL}/api/technicians/profile/{tech_user['id']}")
        assert r.status_code == 200, r.text
        body = r.json()
        assert body.get("user_id") == tech_user["id"]
...
class TestReviewsPrivacy:
    def test_client_can_view_technician_reviews(self):
        ...
        r = requests.get(f"{BASE_URL}/api/reviews/user/{tech_user['id']}", headers=h_client)
        assert r.status_code == 200, r.text
        body = r.json()
        assert "reviews" in body
    ...
    def test_technician_can_view_client_reviews(self):
        ...
        r = requests.get(f"{BASE_URL}/api/reviews/user/{client_user['id']}", headers=h_tech)
        assert r.status_code == 200, r.text
    ...
    def test_user_can_view_own_reviews(self):
        _, client_user, h = _login("cliente@test.com", "test123")
        r = requests.get(f"{BASE_URL}/api/reviews/user/{client_user['id']}", headers=h)
        assert r.status_code == 200, r.text
...
class TestPriceCalculation:
    ...
    def test_base_price_within_6km(self):
        ...
        from services.pricing import calcular_precio as calculate_visit_price
        total = calculate_visit_price(5.0)
        assert total == 9990.0
    ...
    def test_price_above_6km(self):
        ...
        sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
        from services.pricing import calcular_precio as calculate_visit_price
        total = calculate_visit_price(10.0)
    ...
    def test_price_exactly_6km(self):
        ...
        sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
        from services.pricing import calcular_precio as calculate_visit_price
        total = calculate_visit_price(6.0)
        assert total == 9990.0
